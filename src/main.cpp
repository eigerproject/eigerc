...
void RunSource(const std::string &src, CompilerContext &ctx,
    ...
    if (ctx.cmdOpts.verbose) root->PrettyPrint();
...
void RunFile(const std::string &filepath, const CmdOptions &opts) {
...
void RunShell(const CmdOptions &opts) {
    CompilerContext ctx(opts);
    std::shared_ptr<Scope> globalScope = std::make_shared<Scope>(ctx);
    SetupCompiler(ctx, globalScope);
...
const std::string_view version = "v0.7.3";
...
static void PrintInfo() {
    ...
    std::string compilerInfo = "Clang " + std::to_string(__clang_major__) +
